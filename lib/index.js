...
module.exports = {
  ...
  auth: {
    ...
    autoThumb: {
      label:
        "VIDEO FILE ONLY: Automatically generate thumbnails for video (supported format .mp4, thumbnail size 480x360)",
      type: "enum",
      values: ["no", "yes"],
    },
    timeout: {
  ...
  init: (config) => {
    const ossClient = new OSS({
      ...
      endpoint: process.env.NODE_ENV !== "production" ? "oss-cn-hongkong-internal.aliyuncs.com" : "oss-accelerate.aliyuncs.com",
    ...
    return {
      upload: (file) => {
        ...
        return new Promise((resolve, reject) => {
          ...
          const fullVersionFileName = `${file.hash}-full-${Date.now()}${
            file.ext
          }`;
          ...
          const generateThumbnail = () => {
            which("ffmpeg", (err) => {
              if (!err) {
                ...
                ossClient
                  .putStream(
                    `${path}${file.hash}-${THUMBNAIL_SIZE}.png`,
                    proc.stdout
                  )
                  .then((result) => {
                    // delete tmp file
          ...
          if (file.ext === ".wav") {
            ...
            encoder.encode().then(() => {
              ...
              ossClient
                .put(shortPath, cutbufferMp3, putOptions)
                .then((result) => {
                  ...
                  ossClient
                    .put(fullPath, fileBuffer, putOptions)
                    .then((result) => {
          ...
          ossClient
            ...
            .then((resp) => {
              ...
              if (resp.res && /2[0-9]{2}/.test(resp.res.statusCode)) {
                ...
                ossClient
                  .delete(`${path}${file.hash}-${THUMBNAIL_SIZE}.png`)
